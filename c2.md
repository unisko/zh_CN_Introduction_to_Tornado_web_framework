...
现在，对Tornado模板系统的基本特性有了掌握。对于许多简单web应用，比如Alpha Munger，这些基本特性就够了。但关于模板的内容不止于此。Tornado还有一些以代码块及模块形式的技巧，代码块和模块两种特性令到复杂web应用的编写更为容易（Tornado still has a few template tricks up its sleeve in the form of blocks and modules, two features that make it easier to write and maintain sophisticated web applications）。在第三章就会研究这些特性。
