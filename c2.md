...
编写web应用时，会经常想要提供“静态内容”（static content），比如样式表、JavaScript文件以及图像等，而又不愿为这些文件编写单独地编写请求处理器。
