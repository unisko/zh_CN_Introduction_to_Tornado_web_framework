...
这里的`handlers`参数是重要的，值得更详细地进行了解。**handlers参数应是一些元组的列表**（a list of tuples），列表中的每个元组都包含了一个用于匹配的正则表达式(a regular expression to match)作为该元组的第一个成员，以及一个`RequestHandler`类作为该元组的第二个成员。在`hello.py`程式中，仅指定了一个的正则表达式、`RequestHandler`对，但可依所需，将任意多这样的对，放入到该列表中。
