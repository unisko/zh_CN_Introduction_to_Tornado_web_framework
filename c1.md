...
Tornado使用这些元组中的正则表达式，来对HTTP请求中的路径（*path*）进行匹配。（所谓**路径，就是URL中位于主机名之后的部分，去除查询字串及片段**（fragment, **译者注：**"#"后面的字串））Tornado在对待这些正则表达式时，就好象这些正则表达式包含了行开始及行结束锚点似的（比如，字串“/”会认为是“^/$”, **译者注：** 关于[regex anchors](http://www.rexegg.com/regex-anchors.html)）。
