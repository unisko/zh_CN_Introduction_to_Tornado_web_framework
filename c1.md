...
这些就是实际令到Tornado应用运行起来的那些行。首先，__使用Tornado的`options`库文件，来解析命令行输入__。接着__创建出一个Tornado `Application`类的实例__（aka. 对象）。而__传递给`Application`类`__init__`方法的最重要参数，就是那些`handlers`处理器__了。该参数__告诉Tornado，使用哪些类来处理相应的HTTP请求__。关于这点，马上就会谈到。
